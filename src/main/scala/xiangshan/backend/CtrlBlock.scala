...
//  dispatch.io.singleStep := RegNext(io.csrCtrl.singlestep)
  ...
  val backendintDqAndintFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.ctrl.commitType)) && !ren.ready && !rename.io.enqRob.canAccept && !intDq.io.enq.canAccept && fpDq.io.enq.canAccept && lsDq.io.enq.canAccept && !rename.io.intFlCanAccept && rename.io.intFlCanAccept && !rename.io.rabCommits.isWalk})  )
  val backendlsDqAndintFlStall = PopCount( (rename.io.in map { ren =>  (ren.valid || CommitType.isFused(ren.bits.ctrl.commitType)) && !ren.ready && !rename.io.enqRob.canAccept && intDq.io.enq.canAccept && fpDq.io.enq.canAccept && !lsDq.io.enq.canAccept && !rename.io.intFlCanAccept && rename.io.intFlCanAccept && !rename.io.rabCommits.isWalk})  )
