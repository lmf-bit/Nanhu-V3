...
class IntegerReservationStationImpl(outer:IntegerReservationStation, param:RsParam) extends LazyModuleImp(outer)
  ...
  private val stdSelectNetwork = Module(new OldestSelectNetworkInt(param.bankNum, entriesNumPerBank, stdIssuePortNum, stdExuCfg, false, Some(s"IntStdSelNetwork")))
