...
class RobImp(outer: Rob)(implicit p: Parameters) extends LazyModuleImp(outer)
  ...
  val numWbPortsWithRedirect = writebackIn.count(_._1.hasRedirectOut)
  ...
  val writebackStd = writebackIn.filter(_._1.exuType == ExuType.stdi) ++ writebackIn.filter(_._1.exuType == ExuType.stdf)
  val writebackNoStd = writebackIn.filter(wb => wb._1.exuType != ExuType.stdi && wb._1.exuType != ExuType.stdf)
  val wbPorts = writebackNoStd.map(_._2)
  ...
  */
  ...
  io.commits.isExtraWalk := (state === s_extrawalk)
  ...
  for ((cfg, wb) <- writebackNoStd) {
    ...
    if(cfg.hasRedirectOut) {
      ...
      wbStatusArray.redirect(hasRedirect, ri.robIdx, false.B)
    }
  }
...
//      debug_microOp(wbIdx).cf.predebugInfo.decodeTime := wb.bits.uop.cf.predebugInfo.decodeTime
  ...
  private val lqDeqSafeLimit = RegNext(enqPtr - 1.U)
  private val lqSafeBlocked = Seq.tabulate(CommitWidth)(idx => {
    val res = Wire(Bool())
    val presentPtr = lqSafeDeqPtr + idx.U
    val nextPtr = lqSafeDeqPtrNext + idx.U
    entryDataModule.io.raddr(idx + CommitWidth) := nextPtr.value
    val entry = entryDataModule.io.rdata(idx + CommitWidth)
    res := (entry.commitType === CommitType.STORE && !writebacked(presentPtr.value)) || (presentPtr >= lqDeqSafeLimit)
    res
  }) :+ true.B
  ...
  val specialRedirectPoint = RegInit(false.B)
  when(io.redirect.valid){
    specialRedirectPoint := enqPtr.value === deqPtr.value && io.redirect.bits.robIdx.value === deqPtr.value && io.redirect.bits.flushItself()
  ...
  for (i <- 0 until RobSize) {
   val needFlush = redirectValidReg && Mux(
    (redirectEnd > redirectBegin) && !specialRedirectPoint,
  ...
  for (i <- 0 until RenameWidth) {
    ...
    val compressWbNum = io.enq.req(i).bits.compressWbNum
  ...
  wbStatusArray.redirect(exceptionGen.io.out.valid, exceptionGen.io.out.bits.robIdx, true.B)
  when(exceptionGen.io.out.valid) {
    val wbIdx = exceptionGen.io.out.bits.robIdx.value
    store_data_writebacked(wbIdx) := true.B
  }

  writebackStd.map(_._2).zipWithIndex.foreach {
    case (wb, i) => {
      when(wb.valid) {
        store_data_writebacked(wb.bits.uop.robIdx.value) := true.B
      }
    }
  }
  ...
  */
  ...
  entryDataModule.io.raddr.take(CommitWidth).zip(commitReadAddr_next).foreach({case(a, b) => a := b})
  ...
  */
  ...
  XSPerfAccumulate("commitCompressCntAll", PopCount(io.commits.commitValid.zip(io.commits.info).map { case (valid, info) => io.commits.isCommit && valid && (info.instrSize.getOrElse(0.U) > 1.U) }))
  (2 to RenameWidth).foreach(i =>
    XSPerfAccumulate(s"commitCompressCnt${i}", PopCount(io.commits.commitValid.zip(io.commits.info).map { case (valid, info) => io.commits.isCommit && valid && (info.instrSize.getOrElse(0.U) === i.U) }))
  )
  XSPerfAccumulate("compressSize", io.commits.commitValid.zip(io.commits.info).map { case (valid, info) => Mux(io.commits.isCommit && valid && (info.instrSize.getOrElse(0.U) > 1.U), info.instrSize.getOrElse(0.U), 0.U) }.reduce(_ +& _))
  ...
  val debug_hasEnq = io.enq.canAccept && io.enq.req.map(req => req.valid && req.bits.firstUop).reduce(_||_)
  when(debug_hasEnq) {
    val enqVec = io.enq.req.map(req => req.valid && req.bits.firstUop)
