...
//  val d = Output(Bool())    // Debug
  ...
  vsetFu.io.in.valid := io.in.valid && (io.in.bits.uop.ctrl.fuOpType === CSROpType.vsetivli || io.in.bits.uop.ctrl.fuOpType === CSROpType.vsetvli ||  io.in.bits.uop.ctrl.fuOpType === CSROpType.vsetvl)
  ...
  when(csrio.vcsr.robWb.vxsat.valid) {
    vcsr := vxsat_wfn(update = true)(csrio.vcsr.robWb.vxsat.bits)
  }
  ...
  val intrNOReg = RegEnable(intrNO, intrBitSet)
  ...
  val difftestExptNO = Mux(!hasIntr, causeNO, 0.U)
  ...
  if(env.EnableDifftest || env.AlwaysBasicDiff) {
    ...
    difftestVCSR.vxsat := Mux(csrio.vcsr.robWb.vxsat.valid, Cat(0.U((XLEN-1).W), csrio.vcsr.robWb.vxsat.bits(0) || vcsr(0)), Cat(0.U((XLEN-1).W), vcsr(0)))
